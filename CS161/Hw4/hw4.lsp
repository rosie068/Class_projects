...
;
; returns list of values representing colors n can take CNF
...
;
; helper returns list of tuples representing colors n can take at most once
; shoud return the list ((-c -a) (-c -(a+1)) (-c -(a+2)) .... (-c -k))
(defun at-most-help (n c a k)
  (cond ((> a k) nil)
    (t (cons (list (- (node2var n c k)) (- (node2var n a k))) (at-most-help n c (+ a 1) k)))
  )
)

; returns list of tuples that n can at most take once
; propositional logic equivalence of list of tuples CNF
(defun at-most-one-color (n c k)
  (cond ((= c k) nil)
    (t (append (at-most-help n c (+ c 1) k) (at-most-one-color n (+ c 1) k)))
  )
)
...
;
; returns list of at least and at most to make sure n can take one color and one only
...
;
; helper for constraints on both nodes at the ends of the edge
(defun generate-edge-help (e k a)
  (cond ((> a k) nil)
    (t (cons (list (- (node2var (first e) a k)) (- (node2var (second e) a k))) (generate-edge-help e k (+ a 1))))
  )
)

; list of clauses for constraints on edge e
(defun generate-edge-clauses (e k)
  (generate-edge-help e k 1)
)
